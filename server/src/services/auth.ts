...
const SECRET_KEY = new TextEncoder().encode(
  process.env.JWT_SECRET || 'pingup-secret-key-change-in-production'
);
...
export async function registerUser(
  username: string
): Promise<{ userId: string; secret: string; qrCode: string }> {
...
export async function verifyToken(
  token: string
): Promise<{ sub: string; username: string; role: string; sessionId: string } | null> {
  try {
